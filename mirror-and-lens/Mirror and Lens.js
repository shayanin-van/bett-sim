...
let simBox = document.getElementById('simBox');
...
class OptDevice {
    constructor(x, y, f, index) {
        ...
        simBox.appendChild(this.inputDiv);
    }

    // update for each animation frame
    update() {
        this.f = this.slider.value;
        this.inputDiv.style.left = this.x*PpCM - 56 + 'px';
        this.inputDiv.style.top = this.y*PpCM - deviceHalfD*PpCM - 117 + 'px';
...
class Object {
    constructor(x, y) {
        ...
        this.stemIsHovered = false;
        this.stemIsSelected = false;
        this.stemIsDragged = false;
    ...
    createShapePath() {
        context.beginPath();
        context.arc(this.x*PpCM, this.y*PpCM, objClickableAreaRadius + 9, 0, 2*Math.PI, false); // plus 12 pixels for radius to make it easier to hit the object with small mobile screen
    }

    createStemShapePath() { // path for dragging the stem part and not alter the object's height
        context.beginPath();
        if (this.y < principalAxis - 10/PpCM) {
            context.moveTo(this.x*PpCM + 5, this.y*PpCM + 10);
            context.lineTo(this.x*PpCM + 5, principalAxis*PpCM);
            context.lineTo(this.x*PpCM - 5, principalAxis*PpCM);
            context.lineTo(this.x*PpCM - 5, this.y*PpCM + 10);
        } else if (this.y > principalAxis + 10/PpCM) {
            context.moveTo(this.x*PpCM + 5, this.y*PpCM - 10);
            context.lineTo(this.x*PpCM + 5, principalAxis*PpCM);
            context.lineTo(this.x*PpCM - 5, principalAxis*PpCM);
            context.lineTo(this.x*PpCM - 5, this.y*PpCM - 10);
...
function onEachStep() {
    ...
    for (let i = 0; i < equipments.length; i++) {
        ...
        if (equipments[i].isHovered || equipments[i].isSelected || equipments[i].stemIsHovered || equipments[i].stemIsSelected) {
...
// scale the fixed-size stage to fill the viewport, keeping its aspect ratio
function fitStage() {
    let scale = Math.min(window.innerWidth/simBox.offsetWidth, window.innerHeight/simBox.offsetHeight);
    simBox.style.transform = 'translate(-50%, -50%) scale(' + scale + ')';
}
...
function onMove(evt) {
    for (let i = 0; i < equipments.length; i++) {
        if (i == 0) {
            ...
            equipments[i].createStemShapePath();
            if (context.isPointInPath(convToCanvasX(evt.pageX), convToCanvasY(evt.pageY))) {
                equipments[i].stemIsHovered = true;
            } else {
                equipments[i].stemIsHovered = false;
            }
...
function onClick(evt) {
    for (let i = 0; i < equipments.length; i++) {
        if (i == 0) {
            ...
            if (equipments[i].stemIsHovered) {
                equipments[i].stemIsSelected = true;
                equipments[i].stemIsDragged = true;
                canvasTop.addEventListener('pointerup', () => {
                    equipments[i].stemIsDragged = false;
                    equipments.sort((a, b) => a.x - b.x); // make sure the equipments is sorted from left to right after dragging
                    equiNum = createNumberArray(equipments.length);
                }, {once: true});
            } else {
                equipments[i].stemIsSelected = false;
            }
        } else {
            // fix for making touch device logic make sense
            equipments[i].createShapePath();
...
// convert window x coordinate to canvas x coordinate (the stage is scaled, so scale the offset back)
function convToCanvasX(winX) {
    let rect = canvas.getBoundingClientRect();
    return (winX - rect.left - window.scrollX)*canvas.width/rect.width;
}

// convert window y coordinate to canvas y coordinate
function convToCanvasY(winY) {
    let rect = canvas.getBoundingClientRect();
    return (winY - rect.top - window.scrollY)*canvas.height/rect.height;
}
...
function writeTextForWhichImageIsDisplaying(number) {
    ...
    contextTop.textAlign = 'center'; // nothing else draws on contextTop, so this never needs resetting
    if (number == 0) {
        contextTop.fillText('All images', 600, 673);
    } else {
        contextTop.fillText('Formation ' + number, 600, 673);
    }
}

function drawFocusAndCenter(device) {
    let radiusSize = 12; // in pixels
    ...
    context.fillText("F", f1X - 5, yToDraw + 6);
    context.fillText("F", f2X - 5, yToDraw + 6);
    if (device.isConcMir.checked || device.isConvMir.checked) {
        context.fillText("C", c1X - 6, yToDraw + 6);
        context.fillText("C", c2X - 6, yToDraw + 6);
    } else {
        context.fillText("2F", c1X - 10, yToDraw + 6);
        context.fillText("2F", c2X - 10, yToDraw + 6);
    }
}
...
function drawVirtualObjectWarning() {
    ...
    context.fillText('!  Virtual object case (s < 0)', 435, 540);
    context.fillText('Ray diagram cannot be drawn', 435, 570);
}

function drawLabels() {
    function writeText(xPos, yPos, color, text) {
        context.font = '22px Mitr';
        context.fillStyle = color;
        context.strokeStyle = '#000000';
        context.lineWidth = 0.5;
        context.textAlign = 'center';
        if (yPos <= principalAxis) {
            context.fillText(text, xPos*PpCM, yPos*PpCM - 14);
            context.strokeText(text, xPos*PpCM, yPos*PpCM - 14);
        } else {
            context.fillText(text, xPos*PpCM, yPos*PpCM + 28);
            context.strokeText(text, xPos*PpCM, yPos*PpCM + 28);
        }
        context.textAlign = 'left';
    }

    writeText(equipments[0].x, equipments[0].y, '#ffffff', 'Object');
    if (imageToDisplay > images.length) {
        imageToDisplay = images.length;
    }
    if (images.length == 1) {
        writeText(images[0].x, images[0].y, colorCycle[0], 'Image');
    } else {
        if (imageToDisplay == 0) {
            for (let i = 0; i < images.length; i++) {
                writeText(images[i].x, images[i].y, colorCycle[i % (colorCycle.length)], 'Image ' + (i + 1));
            }
        } else {
            writeText(images[imageToDisplay - 1].x, images[imageToDisplay - 1].y, colorCycle[(imageToDisplay - 1) % (colorCycle.length)], 'Image ' + imageToDisplay);
            if (imageToDisplay >= 2) {
                writeText(images[imageToDisplay - 2].x, images[imageToDisplay - 2].y, colorCycle[(imageToDisplay - 2) % (colorCycle.length)], 'Image ' + (imageToDisplay - 1));
            }
        }
    }
}
